...
import com.crsocial.witchhatatelier.spell.SpellPipelineRunner;
 ...
 * </ol>
 *
 * <p>Spell recognition and compilation are delegated to {@link SpellPipelineRunner}.
 * Activation sound/particle effects are delegated to {@link SpellActivationEffects}.</p>
 */
public final class SaveGestureHandler {
    ...
    public static void handle(final SaveGesturePayload payload, final IPayloadContext context) {
        context.enqueueWork(() -> saveGesture(payload, context.player()));
        context.enqueueWork(() -> SpellPipelineRunner.run(payload, context.player()));
        context.enqueueWork(() -> SpellActivationEffects.play(payload, context.player()));
    }

    // ── Save paths ────────────────────────────────────────────────────────────────

    private static void saveGesture(SaveGesturePayload payload, Player player) {
        // ── Block-entity path ──────────────────────────────────────────────
